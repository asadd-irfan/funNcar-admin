        ...
        try {
            await API.get(APIUrls.GetMe).then(result => {
                ...
                if (result.data.role === 'admin') {
                    ...
                    dispatch({ type: SHOW_ALERT, payload: { type: 0, message: "Unauthorized role." } });
                }
            });
        } catch (error) {
            // console.log(error)
            dispatch({ type: AUTH_ERROR });
            dispatch({ type: UPDATE_LOADING, payload: false });
            dispatch({ type: SHOW_ALERT, payload: { type: 0, message: error?.response?.data?.error ? error?.response?.data?.error : APP_ERROR_MSGS.StandardErrorMsg } });
        }
    }
    else {
        dispatch({ type: UPDATE_LOADING, payload: false });
    ...
    try {
        await API.get(APIUrls.getAppConfigs).then(result => {
            if (result.data.status === true) {
                ...
                dispatch({ type: SHOW_ALERT, payload: { type: 0, message: result.data.message } });
            }
        });
    } catch (error) {
        dispatch({ type: SHOW_ALERT, payload: { type: 0, message: error?.response?.data?.error ? error?.response?.data?.error : "Error in getting app configs" } });
    }
};
