...
import { getHomePageData, deleteHomePageFile, uploadHomePageFile, updateHomePageData, getHomePopularPerformer } from '../../actions/homepage';
...
export default function PerformerHomePages() {
     ...
     const getPopularFunncar = () => {
         dispatch({ type: UPDATE_LOADING, payload: true });
         getHomePopularPerformer().then(result => {
             dispatch({ type: UPDATE_LOADING, payload: false });
     ...
     const handleInputChange = (e) => {
         if (e.target.files.length > 0) {
             ...
             uploadHomePageFile('performer', formData).then(result => {
                 dispatch({ type: UPDATE_LOADING, payload: false });
                 if (result.data.status === true) {
     ...
     const handleSelectChange = selectedOption => {
        ...
        console.log('selectedOption',selectedOption);
         console.log('popularFunncars',popularFunncars);
         console.log('allPopularFunncars',allPopularFunncars);
         if (selectedOption.length < 11) {
             setSelectedFunncar(selectedOption);
             let funncarArray = [];
             selectedOption.forEach(element => {
                 let found = allPopularFunncars.find(item => item._id.toString() == element.value.toString())
                console.log(found);
                 funncarArray.push({ funncar: found })
             });
             console.log('funncarArray',funncarArray);
                ...
                <GridContainer className="mt-20">
                    ...
                    <GridItem xs={12} sm={12} md={8} className="pt-5 mt-5">
                        ...
                        <Select
                            ...
                            placeholder="All Popular Performers"
                        /> <br />
                        <Button
                            onClick={() => onSavePopularFunncar()}
                            type="button" color="primary" style={{ marginRight: 30, float: 'right' }}>Save Popular Performers</Button>
