...
import { getUsers, deleteUser, filterPerformers } from "../../actions/users";
...
export default function PerformersList(props) {
  // Hooks
  ...
  const handleListBtnClick = (id,type) =>{
    let state ={
      redirectToURL: `/admin/performers`,
      data: { search, country, city, categoryType, filter},
      search: props.location.search ?? "" 
    }
    let pathName=  "";
    if(type === "view") pathName = `/admin/performer/${id}`
    if(type === "edit") pathName = `/admin/Performer_Edit/${id}`
    history.push({
      pathname: pathName,
  ...
  const exportList = () => {
    ...
    filterPerformers(body, paramString)
      .then((result) => {
  ...
  useEffect(() => {
    scrollToDiv("performers-list-container");
  ...
  return (
    <>
      ...
      />
      <GridContainer id="performers-list-container">
